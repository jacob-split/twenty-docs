...
Access your Twenty instance (usually http://localhost:3000)
...
And much more…
Admin Panel Configuration Variables
Each variable is documented with descriptions in your admin panel at Settings → Admin Panel → Configuration Variables. Some infrastructure settings like database connections (PG_DATABASE_URL), server URLs (SERVER_URL), and app secrets (APP_SECRET) can only be configured via .env file.
Complete technical reference →
​
2. Environment-Only Configuration
IS_CONFIG_VARIABLES_IN_DB_ENABLED=false
All configuration managed through .env files:
Set IS_CONFIG_VARIABLES_IN_DB_ENABLED=false in your .env file
Add all configuration variables to your .env file
